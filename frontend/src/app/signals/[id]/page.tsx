...
export default function SignalDetails() {
  ...
  // Add state for V2 access
  const [v2Access, setV2Access] = useState<boolean>(false);
  const [v2Loading, setV2Loading] = useState<boolean>(true);
  ...
  useEffect(() => {
    const checkV2Access = async () => {
      if (user?.wallet?.address) {
        setV2Loading(true);
        try {
          const hasAccess = await UserService.checkV2Access(
            user.wallet.address
          );
  ...
  // If user has V2 access, use V2 layout
  if (v2Access && !v2Loading) {
    return (
